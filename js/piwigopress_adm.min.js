...
}}})}});$(document).ready(function(){var pwgp_Gallery_Display=true;$("a#PWGP_button").click(function(){if(pwgp_Gallery_Display){if($("#dashboard-widgets-wrap").size()==0){var where="#poststuff"
...
$("#PWGP_hide").hide();$("#PWGP_show").hide();$("#PWGP_show_stats").hide()});$("#PWGP_load").click(function(){var url=$("#PWGP_finder").val(),loaded=5,$gallery=$("#PWGP_dragger"),$dragli=$("#PWGP_dragger li"),$trash=$("#PWGP_dropping");
$(".PWGP_system").show(500);$("#PWGP_Load_Active").show();$gallery.load("../wp-content/plugins/piwigopress/thumbnails_reloader.php?&url="+url,function(){$("#PWGP_more").show().click(function(){Get_more()
});$("#PWGP_hide").show().click(function(){var hide=Math.max(1,Math.floor($("li:visible",$gallery).size()/2));for(i=0;i<hide;i++){$gallery.find("li:visible").first().hide()
}if($("li:visible",$gallery).size()==0){$("#PWGP_hide").hide()}else{$("#PWGP_show").show().click(function(){$("li:hidden",$gallery).show();$("#PWGP_show").hide()
})}});Drag_n_Drop();$("#PWGP_Load_Active").hide();function Get_more(){$("#PWGP_Load_Active").show();$.ajax({url:"../wp-content/plugins/piwigopress/thumbnails_reloader.php?&loaded="+loaded+"&url="+url,cache:false,success:function(html){Drag_n_Drop(html)
...
if($hdesc==1){$scode+=" desc=1"}var $hclass=$("#photo_class").val();if($hclass!=""){$scode+=" class='"+$hclass+"'"}var $scode="\t"+$scode+"] \n\r";
$("#content").insertAtCaret($scode);tinyMCE.execInstanceCommand("content","mceInsertContent",false,$scode)})});$("a#PWGP_rst").unbind().click(function(){$("li",$trash).appendTo($gallery);
$("a#PWGP_Gen").hide();$("a#PWGP_rst").hide();$("#PWGP_show_stats").show().find("#PWGP_stats").text(" "+$("li",$gallery).size()+" / "+loaded)
